...
def plot_generation(sol:SolutionArray,title:str,X,Y,f,func_par:dict=None):
    ...
    ax.scatter(X[np.argmax(f(X,Y,func_par)) // len(X), np.argmax(f(X,Y,func_par)) % len(X)], 
            Y[np.argmax(f(X,Y,func_par)) // len(Y), np.argmax(f(X,Y,func_par)) % len(Y)], 
