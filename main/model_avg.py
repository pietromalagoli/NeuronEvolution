...
def generate(par:dict,verb:bool=False) -> NetworkParams:  # JAXXED!
    ...
    #loss = compute_loss(NetPar,par,verb=verb)                      # Compute the loss value of the network 
...
#           print(f'Path cost:{(path_cost)**2}')
        ...
        #loss += target_dist + volume_cost  # take the exponential of the sum of the costs (weighted if needed)
...
def mutation(n:NetworkParams,par:dict,gen_verb:bool=False) -> NetworkParams:
    ...
    #loss_m = compute_loss(NetPar,par,verb=gen_verb)
...
def evolution(par:dict,verb:int=1,early_stop:bool=True,gen_verb:bool=False) -> tuple[list,list]:
    ...
    for _ in range(par['N_sol']):
        ...
        t_dist, volume = compute_loss(sol,par)
        t_dists.append(t_dist)
        volumes.append(volume)
    mean_t_dist = np.mean(t_dists)
    mean_volume = np.mean(volumes)
    for n,sol in enumerate(solutions):
        loss = ((t_dists[n]/mean_t_dist) + (volumes[n]/mean_volume)) / len(par['input_set'])
        solutions[n] = NetworkParams(sol.J,sol.C,sol.B,sol.G,sol.T,sol.N,loss)
